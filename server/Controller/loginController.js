...
module.exports.login = async (req, res) => {
  ...
  try {
    if (emailvalidator.validate(email)) {
      if (email && password) {
        let sql = "select * from tbl_admin_login where email = ? and password = ?";
        ...
        if(result[0]?.role === 0){
          ...
          // let loginSql = "UPDATE tbl_admin_login SET last_login_date = ? WHERE email = ? "
          // let loginResult = await mysqlcon(loginSql, [dateTime, Email])
          let token = jwt.sign(
            // { id: result[0].user_id,role:result[0].role },
            { id: result[0].id,role:result[0].role, Status: result[0].status },
            config.JWT_SECRET,
            {
  ...
  try {
    ...
    if (token) {
      jwt.verify(token, config.JWT_SECRET, async (err, payload) => {
        ...
        let id = payload.id;
        if (id) {
          let sql = "SELECT role from tbl_admin_login WHERE user_id = ?"
